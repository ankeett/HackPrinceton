...
export const register = (name, email, password) => async(dispatch) => {
    try {
        ...
        await axios.post(
            `${host}/api/v1/account/sendactivate`,
            {email},
            config
        )
...
export const activate = (token) => async(dispatch)=>{
    try {
        ...
        const {data} = await axios.put(
            `${host}/api/v1/account/activate/${token}`,
        )
...
// LOGOUT
export const logout = (logged="normal") => async(dispatch) => {
    try {
        const config = {  withCredentials: true};
        await axios.get(`${host}/api/v1/logout`, config);
        if(logged === 'signup'){
            window.location.replace('/seller/register')
        }
        else if(logged === 'signin'){
...
export const resetPassword = (token, password) => async (dispatch)=> {
    try {
        ...
        const {data} = await axios.put(`${host}/api/v1/password/reset/${token}`, {password}, config);
...
export const getMyReview = (productId) =>async(dispatch) => {
    try {
        const config = {headers:{"Content-type":"application/json"},  withCredentials: true};
        dispatch({
            type:FETCH_MY_REVIEW_REQUEST,

        })
        const {data} = await axios.get(`${host}/api/v1/review/me/${productId}`, config);

        dispatch({
            type:FETCH_MY_REVIEW_SUCCESS,
            payload:data
        })
    } catch (error) {
         dispatch({
            type:FETCH_MY_REVIEW_FAIL,
            payload:error
